...
const EdgeMarker = memo(
  ({
    ...
    marker: (typeof regionMarkers)[0]
    delay: number
    onHover: (marker: (typeof regionMarkers)[0] | null) => void
  }) => (
...
interface DottedMapProps {
  width?: number
  height?: number
  onCountryClick?: (countryCode: string, countryName: string, requests: number) => void
}

export default function DottedMap({ width = 1000, height = 560, onCountryClick }: DottedMapProps) {
      ...
      <div className="absolute inset-0 pointer-events-auto">
        ...
        >
          <Geographies geography="https://cdn.jsdelivr.net/npm/world-atlas@2/countries-110m.json">
            {({ geographies }) =>
              geographies.map((geo) => {
                ...
                const hasData = countryRequests[countryCode]
                  ...
                  <Geography
                    ...
                    onClick={() => {
                      if (hasData && onCountryClick) {
                        ...
                        onCountryClick(countryCode, countryName, countryRequests[countryCode].value)
                      }
                    }}
                    style={{
                      default: {
                      ...
                      hover: {
                        fill: hasData ? "rgba(255, 255, 255, 0.05)" : "transparent",
                        stroke: hasData ? "rgba(255, 255, 255, 0.2)" : "transparent",
          ...
          {regionMarkers.map((marker, index) => (
            <EdgeMarker key={marker.id} marker={marker} delay={markerDelays[index]} onHover={handleMarkerHover} />
          ))}
        </ComposableMap>
      </div>
